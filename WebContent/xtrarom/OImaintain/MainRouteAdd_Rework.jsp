<!-- /xtrarom/OImaintain/MainRouteAdd_Rework.jsp -->
...
<%
	...
	ProTestRouteBean[] FTpt = OiMaintainService.RPFTRoute(pd_body,brand);
	StringBuffer FTptHTML = new StringBuffer();
	if (FTptHTML != null) {
		for (int i=0; i<FTpt.length; i++) {
			FTptHTML.append("<option value=\"" + FTpt[i].getRoutename() + "\">" + FTpt[i].getRoutename() + "</option>");
		}
	}
	FTpt = null;
...
function insertRow(){
        ...
        var objCell = objRow.insertCell(0);
           objCell.innerHTML += '<td class="list1"><select name="main_route"/><%= FTptHTML %></select></td>';
        var objCell = objRow.insertCell(1);
           objCell.innerHTML += '<td class="list1"><select name="map_route"/><%= FTptHTML %></select></td>';
...
function can(sid){
	window.location="<html:rewrite page="/OImaintain/mainRouteReActionX.do"/>?sid=" +sid;
}
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="bg1" align="center">
          <tr>
            <td width="100%" height="490" valign="top">
              ...
              <form id="form1" name="form1" action="<html:rewrite page="/OImaintain/addMainRouteReActionX.do"/>" method="post">
                  ...
                  <table id="table1" cellspacing=1 cellpadding=0 class="table2">
                    <thead>
                      <tr class="title1">
                        ...
                        <td height="20" align="left">Rework Route</td>
