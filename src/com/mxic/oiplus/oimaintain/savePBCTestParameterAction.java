...
public class savePBCTestParameterAction extends Action {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (listControl.equals("add_vendor")) {
	        ...
	        String[] w_grade = servletRequest.getParameterValues("w_grade");
	        String[] y_grade = servletRequest.getParameterValues("y_grade");
	        String[] j_grade = servletRequest.getParameterValues("j_grade");
	        String[] k_grade = servletRequest.getParameterValues("k_grade");
	        String[] l_grade = servletRequest.getParameterValues("l_grade");
	        String[] n_grade = servletRequest.getParameterValues("n_grade");
	        String[] b_grade = servletRequest.getParameterValues("b_grade");
	        String[] e_grade = servletRequest.getParameterValues("e_grade");
			String[] s_grade = servletRequest.getParameterValues("s_grade");
			...
			if (listControl.equals("update_data")) {
				PBCService.update_data(sid, pd_body, brand,
						...
						c_grade,w_grade,y_grade, j_grade,k_grade,l_grade,n_grade,b_grade,e_grade,s_grade, comment, "update_cmd", hw_configure);

			} else if (listControl.equals("submit_data")) {
				PBCService.update_data(sid, pd_body, brand,
						version, test_type, be_opt, pin_count, pg_type,
						body_size, tester, site, pg_name, actual_file, pg_id, i_grade,
						c_grade,w_grade,y_grade, j_grade,k_grade,l_grade,n_grade,b_grade,e_grade,s_grade, comment, "submit_cmd", hw_configure);

			}
			try {
				servletResponse.sendRedirect(servletResponse.encodeRedirectURL(servletRequest.getContextPath() + "/OImaintain/pbcTestParameterAction.do?sid="+fm.getSid()));
			} catch (IOException e) {
			...
			PBCService.delete_row(record_id, sid, brand, version, pd_body,null);
			try {
			...
			String brand = fm.getBrand();
			String version = fm.getVersion();
			PBCService.delete_row(record_id, sid, brand, version, pd_body, "all");
			OiMaintainService.submit(sid + "", "TF_TEST_PARAMETER_PBC", "Y");
			servletRequest.setAttribute("sid", sid);
			try {
				servletResponse.sendRedirect(servletResponse.encodeRedirectURL(servletRequest.getContextPath() + "/OImaintain/searchAction.do?sid="+sid));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			//return actionMapping.findForward("update_data_jsp");
			return null;
		} else {
