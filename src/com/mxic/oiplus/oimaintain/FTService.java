...
public class FTService {
  ...
  public static FTTestActionForm getInfo(int sid) {
    ...
    try {
      ...
      String sql = "SELECT * FROM (SELECT a.*, b.package_component\n" +
		 	...
		 	"               WHERE sid='" + sid + "' and a.product_body = b.product_body(+) and a.brand = b.brand(+))";
  ...
  public static FTTestActionForm[] getVendorOIList(int sid) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        bean.setActual_file(pdfService.getFileName(conn, rs.getString("PRODUCT_BODY"),
                                       rs.getString("BRAND"),
  ...
  public static FTTestActionForm[] getInitialInfo(int sid) {
    ...
    try {
      ...
      if (check_tx_boolean) {
        ...
        if (check_first_boolean) {
          ...
          sql = "Insert into tf_test_parameter_ft_tx " +
              ...
              "program_name,i_grade,c_grade,w_grade,y_grade,j_grade,k_grade,l_grade,n_grade,b_grade,e_grade,s_grade,tf_comment,actual_file,pgm_special_control,one_main_pgm_group_version,hw_configure " +
      ...
      //tx中有資料
      /*sql = "SELECT * FROM tf_test_parameter_ft_tx a " +
  ...
  public static FTTestAddActionForm[] gettest_modeInfo(String[] test_mode,
    ...
    try {
      ...
      sql.append("SELECT f.* FROM (SELECT distinct a.program_id,c.body_size," +
                 ...
                 "and b.prm2_code = c.package_type and a.pin_count = c.pin_count " +
                 "and substr(a.product_code,1,4) = c.product_body " +
                 "and substr(a.product_code,5,1) = c.backend_option " +
                 "and a.program_status in ("+pgm_no+") and a.sid = d.pg_sid " +
  ...
  public static boolean insertadd(int sid, String pd_body,
    ...
    try {
      ...
      InsertSQL = "Insert into tf_test_parameter_ft_tx " +
          "(sid,tag,pgm_id,product_body,brand,version,test_type,backend_option," +
          "pin_count,package_type,body_size,tester,site,program_name,actual_file,pgm_special_control,one_main_pgm_group_version,hw_configure,tf_comment,i_grade,c_grade,w_grade,y_grade,s_grade,j_grade,k_grade,l_grade,n_grade,b_grade,e_grade) " +
          "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
      ...
      for (int i = 0; i < record_id.length; i++) {
        ...
        while (rs.next()) {
          if (rs.getInt("total_count") == 0) {
        	  ...
        	  sql1.append(
                  "SELECT i_grade, c_grade, w_grade, y_grade, s_grade, j_grade, k_grade, l_grade, n_grade, b_grade, e_grade, hw_configure, tf_comment FROM tf_test_parameter_ft_tx where sid='" + sid +
                  "' and pgm_id='" + pg_id +
                  "' and product_body='" + pd_body +
                  "' and brand='" + brand +
                  "' and version='" + version +
  ...
  public static FTTestVendorActionForm[] getvendor_Info(
    ...
    try {
      ...
      InsertSQL = "Insert into tf_test_parameter_ft_tx " +
          ...
          "c_grade,w_grade,y_grade,j_grade,k_grade,l_grade,n_grade,b_grade,e_grade,s_grade,tf_comment,actual_file,pgm_special_control,one_main_pgm_group_version,hw_configure) " +
          "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
  ...
  public static boolean exit_conform(int sid, String packageComponent) {
    ...
    try {
      ...
      String SelSQL = "SELECT count(*) as total_count " +
          ...
          ((packageComponent!=null&&packageComponent.equals("M"))?"and TF_BOM_MCP_ROUTE='Y' ":"and TF_BOM_ROUTE='Y' ") +
          "and TF_TEST_PARAMETER_WS='Y' " +
          "and TF_TEST_PARAMETER_FT='Y' and TF_BASIC_INFORMATION='Y' " +
          "and TF_DOCUMENT_LINKAGE='Y' and TF_PROD_WAFERLEVEL='Y' " +
          "and (exists (select 1 from  tf_yield_definition_tx a, tf_prod_waferlevel_tx b" +
          "              where a.sid = b.sid" + 
          "                and a.sid='" + sid + "' " +
          "                and a.facility = 0 " +
          "                and b.checked_flag='Y' )" + 
          "    or" + 
          "     not exists (select 1 from tf_yield_definition_tx c" + 
          "                  where c.sid='" + sid + "' " +
          "                    and c.facility = 0 ) " +
          "     )";
  ...
  public static boolean tf_final_exit(int sid, String packageComponent) {
    ...
    try {
      ...
      sql.append("SELECT count(*) as total_count FROM tf_information where sid='")
      	.append(sid)
      	.append("' and TF_PRODUCT_ROUTE='Y' ")
      	.append(packageComponent!=null&&packageComponent.equals("M")?"and TF_BOM_MCP_ROUTE='Y' ":"and TF_BOM_ROUTE='Y' ")
      	.append("and TF_TEST_PARAMETER_WS='Y' and TF_TEST_PARAMETER_FT='Y' " +
                 "and TF_YIELD_WS='Y' and TF_YIELD_FT='Y' " +
                 "and TF_BASIC_INFORMATION='Y' and TF_DOCUMENT_LINKAGE='Y' " +
                 "and TF_TEST_PARAMETER_PBC='Y' and TF_WIP_CONTROL='Y' and TF_CP_TEST_STEP = 'Y' and TF_PROD_WAFERLEVEL='Y'");
  ...
  public static boolean status_apply(int sid) {
    ...
    public static boolean doublicatePIM(int sid) {
        ...
        try {
            ...
            StringBuffer sql = new StringBuffer();
            sql.append(
                    "SELECT COUNT(*) COUNT FROM TF_TEST_PARAMETER_FT_TX C WHERE C.SID = "+sid+" \n" +
                    "AND C.PGM_ID IN (\n" + 
                    "SELECT A.PREVIOUS_PROGRAM_ID FROM PG_PREVIOUS_PROGRAM A WHERE A.PROGRAM_ID IN ( \n" + 
                    "SELECT DISTINCT(B.PGM_ID) FROM TF_TEST_PARAMETER_FT_TX  B WHERE B.SID = "+sid+") \n" + 
    ...
    public static String doublicatePIMS(int sid) {
        ...
        try {
            ...
            StringBuffer sql = new StringBuffer();
            sql.append(
                    "SELECT DISTINCT PGM_ID FROM TF_TEST_PARAMETER_FT_TX C WHERE C.SID = "+sid+" \n" +
                    "AND C.PGM_ID IN (\n" + 
                    "SELECT A.PREVIOUS_PROGRAM_ID FROM PG_PREVIOUS_PROGRAM A WHERE A.PROGRAM_ID IN ( \n" + 
                    "SELECT DISTINCT(B.PGM_ID) FROM TF_TEST_PARAMETER_FT_TX  B WHERE B.SID = "+sid+") \n" + 
                    "AND A.PROGRAM_MODE = 'PROD' \n" + 
