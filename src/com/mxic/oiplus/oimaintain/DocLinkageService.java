  ...
  public static String getInfo1(int sid) {
    ...
    public static String delete_row(String record_id,
      ...
      try {
        ...
        CallableStatement stmt = conn.prepareCall("call sync_tf_document_linkage_tx("+sid+")" );
        stmt.execute();
        stmt.close();
      } catch (Exception ex) {
        ex.fillInStackTrace();
    ...
    public static String delete_all_row_submit(
            int sid,
            String brand,
            String version,
            String pd_body) {
		...
		try {
			...
			conn1 = DBConnection.getConnection();
			StringBuffer sql1 = new StringBuffer();
			sql1.append("SELECT * FROM tf_document_linkage_tx where sid='" + sid + "'");
			...
			while (rs.next()) {
				if (rs.getString("file_name") == null || rs.getString("file_name").equals("")) {
				} else {
					boolean success = (new File(path + rs.getString("file_name"))).delete();
					TDSLogger.println(success);
					TDSLogger.println("success");
				}
			}
			...
			InsertSQL = "Update tf_information set TF_DOCUMENT_LINKAGE='Y' where sid=? ";
			PreparedStatement ps_insert = conn1.prepareStatement(InsertSQL);
			ps_insert.setInt(1, sid);
	        ps_insert.executeUpdate();
