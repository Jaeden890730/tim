...
public class PBCService {
	...
	public static boolean CheckPBC_TX_Exist(String sid,
			String pro_b,
	...
	//check if any earlier version data of the given product body and brand in tf_test_parameter_pbc
	...
	public static boolean PBCToPBCTx(Connection conn,
		...
		try {
			...
			Sql.append("pin_count,package_type,body_size,tester,site,program_name,i_grade,c_grade,w_grade,y_grade,j_grade,k_grade,l_grade,n_grade,b_grade,e_grade,s_grade,");
			...
			Sql.append("program_name,i_grade,c_grade,w_grade,y_grade,j_grade,k_grade,l_grade, n_grade,b_grade,e_grade, s_grade,tf_comment,actual_file,hw_configure ");
	...
	public static PBCTestParameterForm[] getvendor(String record_id) {
		...
		try {
	        ...
	        w_grade = record_id_str[12];
	        y_grade = record_id_str[13];
	        j_grade = record_id_str[14];
	        k_grade = record_id_str[15];
	        l_grade = record_id_str[16];
	        n_grade = record_id_str[17];
	        b_grade = record_id_str[18];
	        e_grade = record_id_str[19];
			s_grade = record_id_str[20];
			pgm_id = record_id_str[21];
			hw_configure = record_id_str[22];
...
*/
	...
	public static String addvendor_Info(
		...
		for (int i=0; i<sites.size(); i++) {
			try {
				...
				"c_grade,w_grade,y_grade,j_grade,k_grade,l_grade, n_grade,b_grade,e_grade,s_grade,tf_comment,actual_file,hw_configure) " +
				"values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
				...
				if (hw_configure == null || hw_configure.equals("")) {
					...
					ps_insert.setString(28, hw_configure);
				}
				ps_insert.executeUpdate();
			} catch (Exception ex) {
				ex.fillInStackTrace();
				...
				message.append("Insert Fail: " + toString(pgm_id, pd_body, brand, version, test_mode, be_opt,
								Integer.toString(pin_count), pg_type, body_size, tester, site, pg_name, actual_file, i_grade, c_grade, w_grade, y_grade,j_grade,k_grade,l_grade, n_grade,b_grade,e_grade, s_grade, hw_configure) + "\n");
			}
		}
		DBConnection.close(conn);
		return message.toString();
	...
	public static void update_data(String[] sid,
		...
		try {
			...
			conn.setAutoCommit(false);
                        InsSQL = "update tf_test_parameter_pbc_tx set i_grade=?, c_grade=?,w_grade=?, y_grade=?,j_grade=?,k_grade=?,l_grade=?,n_grade=?,b_grade=?,e_grade=?, s_grade=?, tf_comment=?, hw_configure=? " +
			...
			if (pin_count != null) {
				for (int i = 0; i < pin_count.length; i++) {
	                ...
	                ps2.setString(7, l_grade==null?"":l_grade[i]);
	                ps2.setString(8, n_grade==null?"":n_grade[i]);
	                ps2.setString(9, b_grade==null?"":b_grade[i]);
	                ps2.setString(10, e_grade==null?"":e_grade[i]);
					ps2.setString(11, s_grade==null?"":s_grade[i]);
					ps2.setString(12, comment[i].trim());
					ps2.setString(13, hw_configure[i]);
					...
					ps2.setString(22, body_size[i]);
					ps2.setString(23, tester[i]);
					ps2.setString(24, site[i]);
					ps2.setString(25, pg_id[i]);
					ps2.setString(26, pg_name[i]);
                    if (actual_file[i].equals(""))
                        ps2.setString(27, "NULL");
                    else
                        ps2.setString(27, actual_file[i]);
	 ...
	 * @param version
	 * @param pd_body
	 * @param type 空值為原先刪除功能，type為all則為刪除sid的所有資訊
	 */

	public static void delete_row(String record_id, int sid,
			String brand, String version,
			String pd_body,String type) {
		Connection conn = null;
		String be_opt = "";
		String test_mode = "";
		int pin_count = 0;
		...
		try {
			...
			if (type == null) {
				sql = sql + 
						"and product_body=? and brand=? and version=? " +
			...
			TDSLogger.println("delete_row");
				...
				TDSLogger.println("delete_row_all");
			}
			ps.executeUpdate();
		} catch (Exception ex) {
			ex.fillInStackTrace();
	...
	public static String toString(String pgm_id, String pd_body, String brand, String version, String test_mode,
			...
			String pg_name, String actual_file, String i_grade, String c_grade,String w_grade, String y_grade,String j_grade, String k_grade,String l_grade,String n_grade, String b_grade,String e_grade, String s_grade, String hw_configure) {
	    ...
	    str.append("W Grade = '" + w_grade + "',");
	    str.append("Y Grade = '" + y_grade + "',");
	    str.append("J Grade = '" + j_grade + "',");
	    str.append("K Grade = '" + k_grade + "',");
	    str.append("L Grade = '" + l_grade + "',");
	    str.append("N Grade = '" + n_grade + "',");
	    str.append("B Grade = '" + b_grade + "',");
	    str.append("E Grade = '" + e_grade + "',");
		str.append("S Grade = '" + s_grade + "'");
    ...
    public static String[] getTestMode() {
        ...
        try {
            ...
            sql.append("WHERE TAG = 30 ORDER BY ID ");
            conn = DBConnection.getConnection();
            PreparedStatement ps = conn.prepareStatement(sql.toString());
            ResultSet rs = ps.executeQuery();
            ArrayList tmp = new ArrayList();
            while (rs.next()) {
                String testMode = rs.getString("DESCRIPTION");
                tmp.add(testMode);
            }
            if (tmp.isEmpty()) {
                return null;
