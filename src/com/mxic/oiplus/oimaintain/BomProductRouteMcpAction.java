...
public class BomProductRouteMcpAction extends Action {
	public ActionForward execute(ActionMapping actionMapping,
		...
		String d = (String) Request.getParameter("delete"); // flag is Y if come
															// from
															// DeleteBomRouteTxAction
															// .java
		if (d == null)
			d = "N";
		...
		String f = null;
		/*
		 * if the sid doesn't come with the web link then get the sid from
		 * ActionForm ProTestRouteBeanAF
		 */
		//if (k == null) {
		...
		try {
			...
			/*
			 * check if tf_bom_route has data for selected product_code,brand,
			 * and version
			 */
			boolean flag = OiMaintainService.CheckExistBomRouteMcp(sid, pro_b,
					br, version);

			/*
			 * check if tf_bom_route_tx has data for selected
			 * product_code,brand, and version
			 */
			boolean f2 = OiMaintainService
					.CheckExistMcpTX(sid, conn, pro_b, br);

			/* check if ChkProdEpn has data for selected product_code */
			boolean ChkProdEpn = OiMaintainService.ChkProdEpn(pro_b, br, sid,
					version);

			if (d.equals("Y")) {
				bom = OiMaintainService.BomRouteMcpFirstVersion(pro_b, br, sid,
						version); // because delete function, so you must keep
									// right now data. no come from
									// tf_bom_route,ChkProdEpn
				forward = "BomProductRoute";
			} else {

				if (f2) {/*
						 * if tf_bom_route_tx has data, just get the data from
						 * tf_bom_route_tx
						 */
					bom = OiMaintainService.BomRouteMcpFirstVersion(pro_b, br,
							sid, version);
					forward = "BomProductRoute";
				} else {
					/*
					 ...
					 */
					if (flag) {
						OiMaintainService.RouteMcpToRouteMcpTx(pro_b, br, sid,
								version);
						if (ChkProdEpn)
							OiMaintainService.EPNtoBomMcpTx(pro_b, br, conn,
									sid, version);
						bom = OiMaintainService.BomRouteMcpFirstVersion(pro_b,
								br, sid, version);
						forward = "BomProductRoute";
					} else {
						if (!ChkProdEpn) {
							...
							OiMaintainService.RouteScpToRouteMcpTx(pro_b, br, conn, sid,
									version);
							OiMaintainService.EPNtoBomMcpTx(pro_b, br, conn,
									sid, version);
							bom = OiMaintainService.BomRouteMcpFirstVersion(
									pro_b, br, sid, version);
							forward = "BomProductRoute";
						}
					}
				}
		...
		boolean status_apply_bo = FTService.status_apply(Integer.parseInt(fm
				.getSid()));
		/**********************************************************************/
		/*
		 * If status is processing and user is the creator or sponsor1 or
		 * sponsor2, show the modification buttons in jsp pages
		 */// //////
		...
		Request.setAttribute("productType", OiMaintainService
				.getProductType(sid));
