...
public class OIMigrationUploadAction extends Action {
  ...
  public ActionForward execute(ActionMapping actionMapping,
    ...
    FormFile bom_route = oIMigrationUploadActionForm.getTf_bom_route();
    String bom_route_name = StringUtil.Utf8ToBig5(bom_route.getFileName());
    ...
    int bom_route_int = bom_route_name.indexOf(fileExt);
    int bom_route_str_int=bom_route_name.length();
    ...
    if (bom_route_int == -1) {
      ...
      //do上傳檔案及service的動作
      try {
        String file_name = path + bom_route_name;
        //將上傳的檔案存在/citplus/File裡
        FileOutputStream fileOutput = new FileOutputStream(file_name);

        fileOutput.write(bom_route.getFileData());
        fileOutput.flush();
        fileOutput.close();
        bom_route.destroy();
        if (productType.equals("NVM")) {
          if (!OIMigrationService.ins_bom_route_info(pd_body, brand, version, file_name))
            success = success + "BOM table";
        } else {
          if (!OIMigrationService.ins_bom_route_info_ASM(pd_body, brand, version, file_name))
            success = success + "BOM table";
        }
      } catch (Exception e) {
        e.printStackTrace();
    ...
    if (parameter_ws_int == -1) {
      ...
      try {
        ...
        if (productType.equals("NVM")) {
          if (!OIMigrationService.ins_parameter_ws_info(pd_body, brand, version, file_name, vendors, testerType))
          {
            if (!success.equals(""))
              success = success + ",";
          ...
          if (!OIMigrationService.ins_parameter_ws_info_ASM(pd_body, brand, version, file_name))
          {
            if (!success.equals(""))
              success = success + ",";
            success = success + "WS Test Parameter";
    ...
    if (parameter_ft_int == -1) {
      ...
      try {
        ...
        if (productType.equals("NVM")) {
          if (!OIMigrationService.ins_parameter_ft_info(pd_body, brand, version, file_name, vendors, testerType))
          {
            if (!success.equals(""))
              success = success + ",";
          ...
          if (!OIMigrationService.ins_parameter_ft_info_ASM(pd_body, brand, version, file_name))
          {
            if (!success.equals(""))
              success = success + ",";
            success = success + "FT Test Parameter";
    ...
    if (parameter_pbc_int == -1) {
      ...
      try {
        ...
        parameter_ft.destroy();
        if (productType.equals("NVM")) {
          if (!OIMigrationService.ins_parameter_pbc_info(pd_body, brand, version, file_name, vendors))
          {
            if (!success.equals(""))
              success = success + ",";
          ...
          if (!OIMigrationService.ins_parameter_pbc_info_ASM(pd_body, brand, version, file_name))
          {
            if (!success.equals(""))
              success = success + ",";
            success = success + "PBC Test Parameter";
    ...
    if (success.equals(""))
      ...
      servletRequest.setAttribute("information","上傳成功!!但 " + success + " 部份資料有問題!!");
    return actionMapping.findForward("migr");
  }
}
