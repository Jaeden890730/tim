...
public class EifPeisPRMII01 extends SQLStatement
{
...
//  private int[] ifbom_seq = null;
  ...
  private static String eifName = "EIFPEISPRMII01";
  private static String eifPath = "/EIFDATA/FROMTDS/EIFPEISPRMII01/";
	//private static String eifPath = "D:\\EIFDATA\\FROMTDS\\EIFPEISPRMII01\\";
  private static int recordLength = 875;

  public EifPeisPRMII01(){
  }

  public void processPEISPRMII01()
  {
    try
    {
  ...
  private boolean selectBOMData()
  {
    ...
    try
    {
      ...
      queryBuffer.append("NVL(FT_ROUTE,' ') FT_ROUTE,NVL(FT_ROUTE_ADD,' ') FT_ROUTE_ADD,\n");
      ...
      queryBuffer.append(", A.form_factor_name, A.module_option \n");
      queryBuffer.append("FROM IF_TF_BOM_ROUTE A, TF_ROUTE_MASTER B\n");
...
//      queryBuffer.append(" WHERE PRODUCT_TYPE != 'NVM' AND A.FT_ROUTE = B.ROUTE_NAME(+) AND SEQ>= 33281 ORDER BY SEQ");
  ...
  /*private void output()
  {
    ...
    try
    {
      os = new FileOutputStream(eifPath+"TIM01-"+(String)DateUtil.Timestamp2String(new java.util.Date(),"yyyyMMddkkmm"));
...
// Available for PC and AIX
        ...
        if (ifbomroute[i].ROUTE_TYPE == null || ifbomroute[i].ROUTE_TYPE.trim().equals("")){
            routetype_length = 0;
            output.append((ifbomroute[i].ROUTE_TYPE==null?"":ifbomroute[i].ROUTE_TYPE.trim())+padding(32-routetype_length));
        }else{
            String routetype = ifbomroute[i].ROUTE_TYPE.trim();
            String encoding = System.getProperty("file.encoding");
            routetype_length = new String(routetype).getBytes("BIG5").length;
            TDSLogger.println("routetype_length="+routetype_length);
            TDSLogger.println("routetype="+routetype.length());
            
            output.append(routetype+padding(32-routetype_length));
  // Available for PC and AIX
        }  
        //20190111output.append(output_stiring(ifbomroute[i].ROUTE_TYPE,32));
        ...
        //MMS
        output.append(output_stiring(ifbomroute[i].FORM_FACTOR_NAME,4));
        output.append(output_stiring(ifbomroute[i].MODULE_OPTION,2));

        pr.write(output.toString()+ "\r\n");
        TDSLogger.println("EifPeisPRMII01-SNDMSG--------");
		...
		try {
			...
			for (int i = 0; i < ifbomroute.length; i++) {
				...
				if (ifbomroute[i].TF_COMMENT.trim().equals("")) {
					...
					String comments = ifbomroute[i].TF_COMMENT.trim();
					comment_length = new String(comments).getBytes("UTF-8").length;
					TDSLogger.println("comment_length=" + comment_length);
					TDSLogger.println("comments=" + comments.length());
					info.put("TF_COMMENT", comments);
				}
				info.put("MASK_OPTION", ifbomroute[i].MASK_OPTION);
				...
				if (ifbomroute[i].TF_WS_COMMENT.trim().equals("")) {
					comment_length = 0;
					info.put("TF_WS_COMMENT",
							ifbomroute[i].TF_WS_COMMENT.trim());
				} else {
				...
				info.put("BODY_REV", ifbomroute[i].BODY_REV);
				info.put("MASK_OPTION_REV", ifbomroute[i].MASK_OPTION_REV);
				info.put("CODE_NO", ifbomroute[i].CODE_NO);
				if (ifbomroute[i].ROUTE_TYPE == null
						|| ifbomroute[i].ROUTE_TYPE.trim().equals("")) {
					routetype_length = 0;
					info.put("ROUTE_TYPE",
							ifbomroute[i].ROUTE_TYPE == null ? ""
									: ifbomroute[i].ROUTE_TYPE.trim());
				} else {
					String routetype = ifbomroute[i].ROUTE_TYPE.trim();
					routetype_length = new String(routetype).getBytes("UTF-8").length;
					TDSLogger.println("routetype_length=" + routetype_length);
					TDSLogger.println("routetype=" + routetype.length());
					info.put("routetype", routetype);
				}
				...
				info.put("QUALITY_LEVEL_COMMENT",
						ifbomroute[i].QUALITY_LEVEL_COMMENT);
				
		        //MMS
				info.put("FORM_FACTOR_NAME", ifbomroute[i].FORM_FACTOR_NAME);
				info.put("MODULE_OPTION", ifbomroute[i].MODULE_OPTION);
  ...
  private void setTrans(String seq)
  {
    ...
    String sqlstr = "update if_tf_bom_route set trans = 'Y' where seq = ?";
    try {
  ...
  {
    EifPeisPRMII01 prm = new EifPeisPRMII01();

    try	{
      prm.processPEISPRMII01();
    } catch(Exception e) {
      TDSLogger.println(e);
    }
  }
