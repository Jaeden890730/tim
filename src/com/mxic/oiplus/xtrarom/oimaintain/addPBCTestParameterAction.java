...
//    				System.out.println(ps_insert.toString());
    		...
    		message.append("Insert fail: " + PBCService.toString(v_pgm_id, product_body, brand, version, v_test_type, v_backend_option,
					Integer.toString(v_pin_count), v_package_type, v_body_size, v_tester, v_site, v_program_name, v_actual_file, "0", "0", "NA") + "\n");
    	}
    }
	...
	if (errorFlag)
		return actionMapping.findForward("failure");
	try {
		servletResponse.sendRedirect(servletResponse.encodeRedirectURL(servletRequest.getContextPath() + "/OImaintain/pbcTestParameterActionX.do?sid="+sid));
	} catch (IOException e) {
