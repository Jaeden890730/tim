 ...
 /*****************************************************************
  ...
  public static void FT_insert(String sid, String pd_body,
    ...
    try {
      ...
      while (rs.next()) {
        conn_check = DBConnection.getConnection();
        sql_check = "Insert into tf_test_parameter_ft (sid,pgm_id,product_body,brand,version,test_type,backend_option,pin_count,package_type,body_size,tester,site,program_name,i_grade,c_grade,tf_comment,hw_configure) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
        ...
        ps_check.setString(16, rs.getString("tf_comment"));
        ps_check.setString(17, rs.getString("hw_configure"));
        ps_check.executeUpdate();
