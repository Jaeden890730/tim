...
public class saveMainRouteReAction extends Action {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		MainRouteReForm fm = (MainRouteReForm) actionForm;
		servletRequest.setAttribute("mainRouteReBeanX",fm);
		...
		if (listControl.equals("update_data") ||
			...
			if (listControl.equals("update_data")) {
				MainRouteService.update_data(sid, pd_body, version, route_type,
						main_route, map_route, remark, "update_cmd","1");

			} else if (listControl.equals("submit_data")) {
				MainRouteService.update_data(sid, pd_body, version, route_type,
						main_route, map_route, remark, "submit_cmd","1");

			}
			try {
				servletResponse.sendRedirect(servletResponse.encodeRedirectURL(servletRequest.getContextPath() + "/OImaintain/mainRouteReActionX.do?sid="+fm.getSid()));
			} catch (IOException e) {
			...
			String version = fm.getVersion();
                        String route_type = Integer.toString(fm.getRoute_type());
			MainRouteService.reset_tx_re(sid);
			try {
				servletResponse.sendRedirect(servletResponse.encodeRedirectURL(servletRequest.getContextPath() + "/OImaintain/mainRouteReActionX.do?sid="+fm.getSid()));
			} catch (IOException e) {
				// TODO Auto-generated catch block
