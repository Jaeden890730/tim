...
public class MakePDFVendorDiff {
  ...
  public static boolean makePDF(Connection conn, Fw8049MainActionForm fm, Document document, String status) {
    boolean IdFlag = true;
...
//                           PdfWriter.AllowPrinting);
      ...
      document.add(new Paragraph(fm.getProduct_body()+ " / "+ fm.getBrand()+" Vendor Differences between version "+ fm.getVersion() +" and "+oldV));
      document.add(new Paragraph("1. All Vendors "));
      PdfPTable table1 = new PdfPTable(1);
      table1.setSpacingBefore(5);
      table1.setWidthPercentage(100);
      table1 = PDFdiffService.SubCon(table1, BasicFont, sid, status,pd_body,brand,version,conn);
      document.add(table1);
      ...
      table2=PDFdiffService.NewVendor(table2, BasicFont, sid, status,pd_body,brand,version,conn);
      document.add(table2);
      ...
      if (!version.equals("0")) {
        ...
        table3 = PDFdiffService.OldVendor(table3, BasicFont, sid, status, pd_body, brand, version,conn);
        document.add(table3);
      }
    } catch (Exception ex) {
//      document.close();
