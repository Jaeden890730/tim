...
//
//	public static String[] GetMMSColData(Connection conn, String pgm_id) {
//		// Unique Key: PROGRAM_ID, PROGRAM_MODE
//		String sql = "select form_factor_name, module_option "
//				+ " from pg_test_program where PROGRAM_ID = '" + pgm_id + "' "
//				+ " and program_mode = 'PROD'";
...
//			if (rs.next()) {
//				formFactorName = rs.getString("form_factor_name");
//				modelOption = rs.getString("module_option");
//			}
//		} catch (Exception ex) {
//			ex.fillInStackTrace();
//			TDSLogger.println("SQL: " + sql + "\nError: " + ex.getMessage());
//		}
//		return new String[] { formFactorName, modelOption };
//	}
//
//	/*****************************************************************
//	 * 主題:取得sid之基本資料
...
//	 *****************************************************************/
	public static FTTestActionForm[] getVendorOIList(int sid) {
		...
		try {
			String sql = "select distinct site, product_body, brand, version, lower(b.tim_short_name) sname from tf_test_parameter_ft_tx a, ba_plant b "
					+ "\n where a.site = b.plant_name and sid = " + sid;

			conn = DBConnection.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			ArrayList<FTTestActionForm> tmp = new ArrayList<FTTestActionForm>();
			FTTestActionForm bean = null;
			while (rs.next()) {
				bean = new FTTestActionForm();
				bean.setPlant_name(rs.getString("SITE"));
				bean.setActual_file(pdfService.getFileName(conn,
						rs.getString("PRODUCT_BODY"), rs.getString("BRAND"),
						rs.getString("VERSION"), rs.getString("SITE"), false,
						"_tx"));
				tmp.add(bean);
			}
...
//		Connection conn = null;
//		FTTestActionForm[] m = null;
//		try {
//			conn = DBConnection.getConnection();
//			m = getVendorList(conn, sid);
//		} catch (Exception ex) {
//			ex.fillInStackTrace();
//			TDSLogger.println(ex.getMessage());
//			return null;
...
//	 *****************************************************************/
//
//	public static FTTestActionForm[] getVendorList(Connection conn, int sid) {
//		// Connection conn = null;
//
//		try {
//			// String sql =
//			// "select distinct site, product_body, brand, version, lower(b.tim_short_name) sname from tf_test_parameter_ws_tx a, ba_plant b "
//			// +
//			// "\n where a.site = b.plant_name and sid = " + sid +
...
//					bean = new FTTestActionForm();
//					// 20100830bean.setSid(sid);
//					bean.setSid(String.valueOf(sid));
//					bean.setPd_body(rs.getString("product_body"));
//					bean.setBrand(rs.getString("brand"));
//					bean.setVersion(rs.getString("version"));
//					bean.setPlant_name(rs.getString("site"));
//					bean.setComment(rs.getString("sname")); // set short TIM
//															// name in Comment
//															// fieid
//					tmp.add(bean);
//				}
...
//
//	/*****************************************************************
//	 * 主題:get第一版資料 ****注意：Device_Size尚未得解喔~~
//	 * ****注意：buy_off_mapping之release資料~~尚未得解喔~~ ****注意buy-off資料~~尚未得解喔~~
//	 *****************************************************************/
//
...
//			if (check_tx_boolean) {
//				// _tx中無資料
//				check_first_boolean = check_first(product_body, brand,
//						version - 1);
//				if (check_first_boolean) {
//					ArrayList<FTTestActionForm> tmp = new ArrayList<FTTestActionForm>();
//					return (FTTestActionForm[]) tmp
//							.toArray(new FTTestActionForm[0]);
//				} else {
//					// 為第二版資料,取得sid
//					// version-1的資料,讀出資料存入tf_test_parameter_ft_tx,tag=0
...
//							+ "select "
//							+ sid
//							+ ",'0',NVL(pgm_id, 0),product_body,brand,"
//							+ version
//							+ ",test_type,backend_option,pin_count,package_type,body_size,tester,site,"
//							+ "program_name,i_grade,c_grade,w_grade,y_grade,j_grade,k_grade,l_grade,n_grade,b_grade,e_grade,s_grade,"
//							+ "tf_comment,actual_file,pgm_special_control,one_main_pgm_group_version,hw_configure,"
//							+ "form_factor_name, module_option "
//							+ "FROM tf_test_parameter_ft where product_body='"
...
//					+ "       A.PROGRAM_NAME,\n"
//					+ "       NVL(A.I_GRADE,(SELECT DISTINCT I_GRADE FROM TF_TEST_PARAMETER_FT B, PG_PREVIOUS_PROGRAM C  WHERE B.PRODUCT_BODY = A.PRODUCT_BODY AND B.BRAND = A.BRAND AND B.VERSION = A.VERSION -1  AND A.PGM_ID = C.PROGRAM_ID AND C.CATEGORY IN (1,2) AND C.PREVIOUS_PROGRAM_ID = B.PGM_ID AND C.PROGRAM_MODE = 'PROD' AND C.PREVIOUS_PROGRAM_MODE = 'PROD')) AS I_GRADE,\n"
//					+ "       NVL(A.C_GRADE,(SELECT DISTINCT C_GRADE FROM TF_TEST_PARAMETER_FT B, PG_PREVIOUS_PROGRAM C  WHERE B.PRODUCT_BODY = A.PRODUCT_BODY AND B.BRAND = A.BRAND AND B.VERSION = A.VERSION -1  AND A.PGM_ID = C.PROGRAM_ID AND C.CATEGORY IN (1,2) AND C.PREVIOUS_PROGRAM_ID = B.PGM_ID AND C.PROGRAM_MODE = 'PROD' AND C.PREVIOUS_PROGRAM_MODE = 'PROD')) AS C_GRADE,\n"
...
//					+ "       NVL(A.B_GRADE,(SELECT DISTINCT B_GRADE FROM TF_TEST_PARAMETER_FT B, PG_PREVIOUS_PROGRAM C  WHERE B.PRODUCT_BODY = A.PRODUCT_BODY AND B.BRAND = A.BRAND AND B.VERSION = A.VERSION -1  AND A.PGM_ID = C.PROGRAM_ID AND C.CATEGORY IN (1,2) AND C.PREVIOUS_PROGRAM_ID = B.PGM_ID AND C.PROGRAM_MODE = 'PROD' AND C.PREVIOUS_PROGRAM_MODE = 'PROD')) AS B_GRADE, \n"
//					+ "       NVL(A.E_GRADE,(SELECT DISTINCT E_GRADE FROM TF_TEST_PARAMETER_FT B, PG_PREVIOUS_PROGRAM C  WHERE B.PRODUCT_BODY = A.PRODUCT_BODY AND B.BRAND = A.BRAND AND B.VERSION = A.VERSION -1  AND A.PGM_ID = C.PROGRAM_ID AND C.CATEGORY IN (1,2) AND C.PREVIOUS_PROGRAM_ID = B.PGM_ID AND C.PROGRAM_MODE = 'PROD' AND C.PREVIOUS_PROGRAM_MODE = 'PROD')) AS E_GRADE\n"
//					+ ",form_factor_name, module_option"
...
//			ArrayList<FTTestActionForm> tmp = new ArrayList<FTTestActionForm>();
//			// String productType =
//			// OiMaintainService.getProductType(Integer.toString(sid));
//
//			while (rs.next()) {
...
//				bean.setOne_main_pgm_group_version(rs
//						.getString("one_main_pgm_group_version"));
...
//				bean.setForm_factor_name(rs.getString("form_factor_name"));
//				bean.setModule_option(rs.getString("module_option"));
//				tmp.add(bean);
//			}
//			return (FTTestActionForm[]) tmp.toArray(new FTTestActionForm[0]);
//		} catch (Exception ex) {
...
//	 *****************************************************************/
//	public static boolean check_first(String product_body, String brand,
//			int version) {
//
...
//
//		try {
//			String sql_check_first = "SELECT count(*) as total_count FROM tf_test_parameter_ft "
//					+ "where product_body='"
//					+ product_body
//					+ "' and brand='"
//					+ brand + "' and version='" + version + "'";
//
//			conn_check_first = DBConnection.getConnection();
//			PreparedStatement ps_check_first = conn_check_first
//					.prepareStatement(sql_check_first.toString());
...
//
//		try {
//			String sql_check_first = "SELECT count(*) as total_count FROM tf_test_parameter_ft_tx "
//					+ "where sid='" + sid + "'";
//			conn_check_first = DBConnection.getConnection();
//			PreparedStatement ps_check_first = conn_check_first
//					.prepareStatement(sql_check_first.toString());
...
//
//	/*****************************************************************
//	 * 主題:取得FTTAdd之test_mode a.取消串ba_package_type table b.將pin_count, body_size,
//	 * package_type欄位取消, 改抓form_factor_name及module_option欄位
//	 *****************************************************************/
//	public static FTTestAddActionForm[] gettest_mode(String brand,
//			String pd_body, String pgmflag) {
//		Connection conn = null;
//		boolean flag = true;
//		String pgm_no = "";
//
...
//			} else {
//				return (FTTestAddActionForm[]) tmp
//						.toArray(new FTTestAddActionForm[0]);
//			}
//		} catch (Exception ex) {
//			ex.fillInStackTrace();
//			TDSLogger.println(ex.getMessage());
...
//			String brand, String pd_body, int sid, String version,
//			String pgmflag) {
//		Connection conn = null;
//		// boolean flag = true;
//		String pgm_no = "";
...
//				} else {
//					test_mode_str = test_mode_str + " or f.test_mode='"
//							+ test_mode[j] + "'";
//				}
//			}
//			conn = DBConnection.getConnection();
...
//			sql_pta.append("        SELECT DISTINCT E.APP_ID APP_ID,\n"
//					+ "                        E.SEQ_NO SEQ_NO,\n"
//					+ "                        PRODUCT_CODE,\n"
//					+ "                        form_factor_name,\n"
//					+ "                        module_option,\n"
...
//
//			PreparedStatement ps_pta = conn
//					.prepareStatement(sql_pta.toString());
//			ResultSet rs_pta = ps_pta.executeQuery();
//			HashMap<String, String> hh_pta = new HashMap<String, String>();
//			String k = "", v = "", v1 = "";
//			while (rs_pta.next()) {
//				k = (rs_pta.getString("PRODUCT_CODE") + '~'
//						+ rs_pta.getString("FORM_FACTOR_NAME") + '~'
//						+ rs_pta.getString("MODULE_OPTION") + '~'
//						+ rs_pta.getString("PLANT_NAME") + '~'
//						+ rs_pta.getString("WIRE_VERSION") + '~'
//						+ rs_pta.getString("TESTER_TYPE") + '~' + rs_pta
//						.getString("BODYSIZE"));
//				v = (rs_pta.getString("APP_ID") + '-'
//						+ rs_pta.getString("SEQ_NO") + "("
//						+ rs_pta.getString("STATUS") + ")");
...
//					+ "e.plant_name,e.plant_no,a.tester_type,a.package_type,a.version,a.actual_file,DECODE(A.DPAT_OPTION,'Y','DPAT_Option=Y;','')||DECODE(A.QT_MERGE_OPTION,'Y','QT_MERGE_OPTION=Y;','')||DECODE(A.FT_ITEM_SAMPLE_OPTION,'Y','FT_ITEM_SAMPLE_OPTION=Y;','')||DECODE(A.SSS_OPTION,'Y','Load sss file=Y;','') pgm_special_control, a.one_main_pgm_group_version, a.os_version "
//					+ ",GET_8049_ONE_MAIN_PGM_NUM(a.one_main_pgm_group_version, '"
//					+ pgmflag
//					+ "') one_main_pgm_num "
//					+ ", a.form_factor_name, a.module_option, a.wire_version "
//					+ "from pg_test_program a, tf_prod_epn c,"
//					+ "pg_plant_release d, ba_plant e "
//					+ "where a.product_code like '"
//					+ pd_body
//					+ "%' "
//					+ "and program_mode = 'PROD' "
//					+ "and a.form_factor_name = c.form_factor_name "
//					+ "and a.module_option =c.module_option "
//					+ "and substr(a.product_code,1,4) = c.product_body "
//					+ "and substr(a.product_code,5,1) = c.backend_option "
//					+ "and a.program_status in ("
//					+ pgm_no
//					+ ") and a.sid = d.pg_sid "
...
//			ResultSet rs = ps.executeQuery();
//			ArrayList<FTTestAddActionForm> tmp = new ArrayList<FTTestAddActionForm>();
//			TDSLogger.println(sql);
//			while (rs.next()) {
//				StringBuffer sql_check = new StringBuffer();
//				sql_check
//						.append("SELECT count(*) as total_count FROM tf_test_parameter_ft_tx where sid='"
//								+ sid
//								+ "' and pgm_id='"
...
//								+ "' and backend_option='"
//								+ rs.getString("be_opt")
//								+ "' and form_factor_name='"
//								+ rs.getString("form_factor_name")
//								+ "' and module_option='"
//								+ rs.getString("module_option")
//								+ "' and body_size='"
...
//						bean.setPg_name(rs.getString("PROGRAM_NAME"));
//						bean.setActual_file(rs.getString("actual_file"));
//						bean.setPgm_special_control(rs
//								.getString("pgm_special_control"));
//						bean.setOne_main_pgm_group_version(rs
//								.getString("one_main_pgm_group_version"));
//						bean.setOne_main_pgm_num(rs
//								.getString("one_main_pgm_num"));
//						bean.setOs_version("");
//						bean.setForm_factor_name(rs
//								.getString("form_factor_name"));
//						bean.setModule_option(rs.getString("module_option"));
//						// if("PK2".equals(bean.getTester()))
//						// bean.setOs_version("PK2 OS Version:"+rs.getString("os_version"));
//
//						k = pd_body + bean.getBe_opt() + "~"
//								+ bean.getForm_factor_name() + "~"
//								+ bean.getModule_option() + "~"
//								+ bean.getSite() + "~"
//								+ rs.getString("wire_version") + "~"
//								+ bean.getTester() + "~" + bean.getBody_size();
//
...
//	 *********************************************************************/
//
//	public static boolean insertadd(int sid, String pd_body, String brand,
//			String version, String[] record_id) {
//
//		int pg_id = 0;
//		String test_type = "";
//		String be_opt = "";
...
//					+ "pgm_special_control,one_main_pgm_group_version,hw_configure,tf_comment,"
//					+ "i_grade,c_grade,w_grade,y_grade,s_grade,j_grade,k_grade,l_grade,n_grade,b_grade,e_grade,"
//					+ "form_factor_name, module_option) "
//					+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
...
//				String[] mmsColData = GetMMSColData(conn, record_id_str[0]);
...
//				StringBuffer sql = new StringBuffer();
//				sql.append("SELECT count(*) as total_count FROM tf_test_parameter_ft_tx where sid='"
//						+ sid
//						+ "' and pgm_id='"
//						+ pg_id
//						+ "' and product_body='"
...
//						sql1.append("SELECT i_grade, c_grade, w_grade, y_grade, s_grade, j_grade, k_grade, l_grade, n_grade, b_grade, e_grade, hw_configure, tf_comment FROM tf_test_parameter_ft_tx where sid='"
//								+ sid
//								+ "' and pgm_id='"
//								+ pg_id
//								+ "' and product_body='"
...
//							w_grade = rs1.getString("w_grade");
//							y_grade = rs1.getString("y_grade");
//							s_grade = rs1.getString("s_grade");
//							j_grade = rs1.getString("j_grade");
//							k_grade = rs1.getString("k_grade");
//							l_grade = rs1.getString("l_grade");
//							n_grade = rs1.getString("n_grade");
//							b_grade = rs1.getString("b_grade");
//							e_grade = rs1.getString("e_grade");
//							hw_configure = rs1.getString("hw_configure");
//							comment = rs1.getString("tf_comment");
//						}
//						String previous_program_id = OiMaintainService
//								.GetProviousProgramId(String.valueOf(pg_id));
//						String previous_hw_configure = OiMaintainService
//								.GetHWConfigureBySidProgramId(
//										"tf_test_parameter_ft_tx",
//										String.valueOf(sid),
//										previous_program_id);
...
//						}
//						if (String.valueOf(pin_count) == null
//								|| String.valueOf(pin_count).equals("")) {
...
//						}
//						if (pgm_special_control == null
//								|| pgm_special_control.equals("")) {
//							ps_insert.setString(16, " ");
//						} else {
//							ps_insert.setString(16, pgm_special_control);
//						}
//						if (one_main_pgm_group_version == null
//								|| one_main_pgm_group_version.equals("")) {
//							ps_insert.setString(17, " ");
//						} else {
//							ps_insert.setString(17, one_main_pgm_group_version);
//						}
//						if (hw_configure == null) {
//							if (previous_hw_configure == null
//									|| previous_hw_configure.equals("")) {
//								ps_insert.setString(18, "");
//							} else {
//								ps_insert.setString(18, previous_hw_configure);
//							}
//						} else {
//							ps_insert.setString(18, hw_configure);
//						}
//						if (comment == null || comment.equals("")) {
//							if (os_version == null || os_version.equals("")) {
//								ps_insert.setString(19, "");
//							} else {
//								ps_insert.setString(19, os_version);
//							}
//						} else {
//							ps_insert.setString(19, comment);
//						}
//						ps_insert.setString(20, i_grade);
//						ps_insert.setString(21, c_grade);
//						ps_insert.setString(22, w_grade);
//						ps_insert.setString(23, y_grade);
//						ps_insert.setString(24, s_grade);
//						ps_insert.setString(25, j_grade);
//						ps_insert.setString(26, k_grade);
//						ps_insert.setString(27, l_grade);
//						ps_insert.setString(28, n_grade);
//						ps_insert.setString(29, b_grade);
//						ps_insert.setString(30, e_grade);
//						ps_insert.setString(31, mmsColData[0]);
//						ps_insert.setString(32, mmsColData[1]);
...
//			ex.printStackTrace();
//			DBConnection.rollback(conn);
//			DBConnection.rollback(conn_exit);
//			ex.fillInStackTrace();
//			TDSLogger.println(ex.getMessage());
//			flag = false;
//			return flag;
...
//	}
//
//	// //select d.* from (SELECT right(left(a.product_code,5),1)as
//	// be_opt,a.*,b.plant_no,c.plant_name,e.status as program_status FROM
//	// pg_test_program a,pg_plant_release b,ba_plant c,buy_off_mapping e where
//	// (a.program_id=b.pg_sid and b.plant_no=c.plant_no and
//	// a.program_status=e.program_status and e.status='release')) d where
//	// d.program_id='2' and program_mode='ENG' and be_opt='6' and test_mode='aa'
//	// and version='99' and program_name='aa' and tester_type='bb' and
//	// package_type='dd' and pin_count='12' and plant_name<>'kkk'
//	// //select distinct d.plant_no,d.plant_name,d.program_status from (SELECT
//	// right(left(a.product_code,5),1)as
//	// be_opt,a.*,b.plant_no,c.plant_name,e.status as program_status FROM
//	// pg_test_program a,pg_plant_release b,ba_plant c,buy_off_mapping e where
//	// (a.program_id=b.pg_sid and b.plant_no=c.plant_no and
//	// a.program_status=e.program_status and e.status='release')) d where
...
//	 *****************************************************************/
//	public static FTTestActionForm[] getvendor(String product_body,
//			String record_id) {
//		Connection conn = null;
//		boolean flag = true;
//		String be_opt = "";
...
//				w_grade = record_id_str[14];
//			if (record_id_str.length > 15)
//				y_grade = record_id_str[15];
//			if (record_id_str.length > 16)
//				j_grade = record_id_str[16];
//			if (record_id_str.length > 17)
//				k_grade = record_id_str[17];
//			if (record_id_str.length > 18)
//				l_grade = record_id_str[18];
//			if (record_id_str.length > 19)
//				n_grade = record_id_str[19];
//			if (record_id_str.length > 20)
//				b_grade = record_id_str[20];
//			if (record_id_str.length > 21)
//				e_grade = record_id_str[21];
//			if (record_id_str.length > 22)
//				s_grade = record_id_str[22];
//			if (record_id_str.length > 23)
//				hw_configure = record_id_str[23];
//			if (record_id_str.length > 24)
//				comment = record_id_str[24];
//			StringBuffer sql = new StringBuffer();
//
//			sql.append("select distinct b.plant_no,c.plant_name,a.program_status "
...
//					+ "and program_mode='PROD' and a.product_code = '"
//					+ product_body
//					+ be_opt
//					+ "' and test_mode='"
//					+ test_mode
...
//					+ site.replaceAll(";", "','") + "')");
//
//			// where be_opt='6' and test_mode='aa' and program_name='aa' and
//			// tester_type='bb' and package_type='dd' and pin_count='12' and
//			// plant_name<>'kkkk'
...
//				bean.setW_grade(w_grade);
//				bean.setY_grade(y_grade);
//				bean.setJ_grade(j_grade);
//				bean.setK_grade(k_grade);
//				bean.setL_grade(l_grade);
//				bean.setN_grade(n_grade);
//				bean.setB_grade(b_grade);
//				bean.setE_grade(e_grade);
//				bean.setS_grade(s_grade);
...
//	}
//
//	public static FTTestVendorActionForm[] getvendor_Info(String[] site_data,
//			String be_opt, String test_mode, int pin_count, String pg_type,
//			String tester, String pg_name, String actual_file,
//			String pgm_special_control, String one_main_pgm_group_version,
//			int sid, String pd_body, String brand, String version,
//			String body_size, String i_grade, String c_grade, String w_grade,
//			String y_grade, String j_grade, String k_grade, String l_grade,
//			String n_grade, String b_grade, String e_grade, String s_grade,
//			String hw_configure, String comment) {
//
//		String InsertSQL = null;
//		Connection conn = null;
...
//				} else {
//					site_data_str = site_data_str + " or d.plant_name='"
//							+ site_data[j] + "'";
//				}
//			}
...
//					+ "c_grade,w_grade,y_grade,j_grade,k_grade,l_grade,n_grade,b_grade,e_grade,s_grade,"
//					+ "tf_comment,actual_file,pgm_special_control,one_main_pgm_group_version,hw_configure,"
//					+ "form_factor_name, module_option) "
//					+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
...
//				StringBuffer sql_check = new StringBuffer();
//				sql_check
//						.append("SELECT count(*) as total_count FROM tf_test_parameter_ft_tx "
//								+ "where sid='"
//								+ sid
//								+ "' and pgm_id='"
//								+ rs.getInt("PROGRAM_ID")
...
//								+ "' and program_name='" + pg_name + "' ");
//
//				PreparedStatement ps_check = conn_check
//						.prepareStatement(sql_check.toString());
//				ResultSet rs_check = ps_check.executeQuery();
//				while (rs_check.next()) {
//					if (rs_check.getInt("total_count") == 0) {
//						// String previous_program_id =
//						// OiMaintainService.GetProviousProgramId(String.valueOf(rs.getInt("PROGRAM_ID")));
//						// String previous_hw_configure = "NA";
//						ps_insert.setInt(1, sid);
//						ps_insert.setString(2, "1");
//						if (rs.getString("PROGRAM_ID") == null
//								|| rs.getString("PROGRAM_ID").equals("")) {
//							ps_insert.setInt(3, 0);
//						} else {
//							ps_insert.setInt(3, rs.getInt("PROGRAM_ID"));
//							// previous_hw_configure =
//							// OiMaintainService.GetHWConfigureBySidProgramId("tf_test_parameter_ft_tx",String.valueOf(sid),rs.getString("PROGRAM_ID"));
//						}
//						ps_insert.setString(4, pd_body);
//						ps_insert.setString(5, brand);
//						ps_insert.setInt(6, Integer.parseInt(version));
...
//							ps_insert.setString(12, tester);
//						}
//						if (rs.getString("plant_name") == null
//								|| rs.getString("plant_name").equals("")) {
//							ps_insert.setString(13, " ");
//						} else {
...
//						ps_insert.setString(17, w_grade);
//						ps_insert.setString(18, y_grade);
//						ps_insert.setString(19, j_grade);
//						ps_insert.setString(20, k_grade);
//						ps_insert.setString(21, l_grade);
//						ps_insert.setString(22, n_grade);
//						ps_insert.setString(23, b_grade);
//						ps_insert.setString(24, e_grade);
//						ps_insert.setString(25, s_grade);
//						ps_insert.setString(26, comment);
//						if (actual_file == null || actual_file.equals("")) {
//							ps_insert.setString(27, " ");
//						} else {
//							ps_insert.setString(27, actual_file);
//						}
//						if (pgm_special_control == null
//								|| pgm_special_control.equals("")) {
//							ps_insert.setString(28, " ");
//						} else {
//							ps_insert.setString(28, pgm_special_control);
//						}
//						if (one_main_pgm_group_version == null
//								|| one_main_pgm_group_version.equals("")) {
//							ps_insert.setString(29, " ");
//						} else {
//							ps_insert.setString(29, one_main_pgm_group_version);
//						}
//						if (hw_configure == null || hw_configure.equals("")) {
//							ps_insert.setString(30, " ");
//						} else {
//							ps_insert.setString(30, hw_configure);
//						}
//						String[] mmsColData = GetMMSColData(conn,
...
//				}
//			}
//			return (FTTestVendorActionForm[]) tmp
//					.toArray(new FTTestVendorActionForm[0]);
//		} catch (Exception ex) {
//			ex.fillInStackTrace();
//			TDSLogger.println(ex.getMessage());
//			flag = false;
...
//	 * 主題:get Add資料
//	 *********************************************************************/
//	public static boolean update_data(String[] sid, String[] pd_body,
//			String[] brand, String[] version, String[] test_type,
//			String[] be_opt, String[] pin_count, String[] pg_type,
//			String[] body_size, String[] tester, String[] site,
//			String[] pg_name, String[] actual_file,
//			String[] pgm_special_control, String[] one_main_pgm_group_version,
//			String[] pg_id, String[] i_grade, String[] c_grade,
//			String[] w_grade, String[] y_grade, String[] j_grade,
//			String[] k_grade, String[] l_grade, String[] n_grade,
//			String[] b_grade, String[] e_grade, String[] s_grade,
//			String[] comment, String[] hw_configure, String flag) {
...
//			InsSQL.append("update tf_test_parameter_ft_tx set i_grade=?, c_grade=?,w_grade=?, y_grade=?,j_grade=?,k_grade=?,l_grade=?,n_grade=?,b_grade=?,e_grade=?, s_grade=?, tf_comment=?, hw_configure=? ");
...
//					DelSQL.append("and site not in ('"
//							+ site[i].trim().replaceAll(";", "','") + "')");
//					PreparedStatement ps0 = conn.prepareStatement(DelSQL
//							.toString());
//					TDSLogger.println(DelSQL.toString());
//					TDSLogger.println(sid[i] + ";" + pg_id[i].trim());
//					ps0.setInt(1, Integer.parseInt(sid[i]));
//					ps0.setString(2, StringUtil.Utf8ToBig5(pg_id[i].trim()));
...
//					if ((actual_file[i] != null)
//							&& (actual_file[i].length() > 0))
//						InsSQL1.append("and actual_file=? ");
//					else
//						InsSQL1.append("and actual_file is null ");
//					ps2 = conn.prepareStatement(InsSQL1.toString());
//
//					ps2.setString(1, i_grade[i]);
//					ps2.setString(2, c_grade[i]);
//					ps2.setString(3, w_grade[i]);
//					ps2.setString(4, y_grade[i]);
//					ps2.setString(5, j_grade[i]);
//					ps2.setString(6, k_grade[i]);
//					ps2.setString(7, l_grade[i]);
//					ps2.setString(8, n_grade[i]);
//					ps2.setString(9, b_grade[i]);
//					ps2.setString(10, e_grade[i]);
//					ps2.setString(11, s_grade[i]);
//					ps2.setString(12, comment[i].trim());
//					ps2.setString(13, hw_configure[i].trim());
//					ps2.setInt(14, Integer.parseInt(sid[i]));
//					ps2.setString(15, pd_body[i]);
//					ps2.setString(16, brand[i]);
//					ps2.setString(17, version[i]);
//					ps2.setString(18, test_type[i]);
//					ps2.setString(19, be_opt[i]);
//					ps2.setString(20, pin_count[i]);
//					ps2.setString(21, pg_type[i]);
//					ps2.setString(22, tester[i]);
//					// ps2.setString(23, site[i]);
//					ps2.setString(23, pg_id[i]);
//					ps2.setString(24, pg_name[i]);
//					if ((body_size[i] != null) && (body_size[i].length() > 0)) {
...
//				InsSQL1 = new StringBuffer(
//						"update tf_information set TF_TEST_PARAMETER_FT = ? ,TF_BASIC_INFORMATION = ? where sid = ? ");
...
//	 * 主題:刪除row資料
//	 *****************************************************************/
//	public static boolean delete_row(String record_list, int sid, String brand,
//			String version, String pd_body) {
//		Connection conn = null;
//		boolean flag = true;
//		String be_opt = "";
//		String test_mode = "";
...
//				i++; // skip temperature
//				i++; // skip temperature (w_grade)
//				i++; // skip temperature (y_grade)
//				i++; // skip temperature (j_grade)
//				i++; // skip temperature (k_grade)
//				i++; // skip temperature (l_grade)
//				i++; // skip temperature (n_grade)
//				i++; // skip temperature (b_grade)
//				i++; // skip temperature (e_grade)
//				i++; // skip temperature (s_grade)
//				i++; // skip temperature (hw_configure)
//				i++; // skip comment (comment)
//				i++; // skip n1_grade (n1_grade)
//
//				ps.setInt(1, sid);
...
//			conn = DBConnection.getConnection();
//			conn.setAutoCommit(false);
//			sql = "delete from tf_test_parameter_ft_tx " + "where sid = ? ";
//			PreparedStatement ps = conn.prepareStatement(sql);
//			ps.setInt(1, sid);
...
//			// "where a.sid='" + sid +
//			// "' and a.pgm_id=b.program_id and b.program_status ='53' " +
...
//	}
//
//	public static boolean exit_conform(int sid, String packageComponent) {
//		boolean flag = false;
//		String InsSQL1 = null;
//		Connection conn = null;
//		Connection conn1 = null;
...
//					+ "FROM tf_information where sid='"
//					+ sid
//					+ "' "
//					+ "and TF_PRODUCT_ROUTE='Y' "
//					+ "and TF_BOM_ROUTE='Y' "
//					+ "and TF_TEST_PARAMETER_FT='Y' and TF_BASIC_INFORMATION='Y' "
...
//	// }
//
//	public static boolean tf_final_exit(int sid, String packageComponent) {
//		Connection conn = null;
//		boolean flag = false;
//
//		try {
...
//			sql.append(
//					"SELECT count(*) as total_count FROM tf_information where sid='")
//					.append(sid).append("' and TF_PRODUCT_ROUTE='Y' ")
//					.append("and TF_BOM_ROUTE='Y' ")
//					.append("and TF_TEST_PARAMETER_FT='Y' ")
//					.append("and TF_YIELD_FT='Y' ")
//					.append("and TF_BASIC_INFORMATION='Y' ")
//					.append("and TF_DOCUMENT_LINKAGE='Y' ");
//
//			PreparedStatement ps = conn.prepareStatement(sql.toString());
//			ResultSet rs = ps.executeQuery();
...
//			StringBuffer sql = new StringBuffer();
//			// ***記得有少找出option及tester的條件喔~~
//			sql.append("SELECT count(*) as total_count FROM tf_information where sid='"
//					+ sid + "' and  status='A' ");
//
//			PreparedStatement ps = conn.prepareStatement(sql.toString());
//			ResultSet rs = ps.executeQuery();
...
//	}
//
//	public static boolean doublicatePIM(int sid) {
//		Connection conn = null;
//		boolean flag = false;
//
//		try {
//			conn = DBConnection.getConnection();
//			StringBuffer sql = new StringBuffer();
//			sql.append("SELECT COUNT(*) COUNT FROM TF_TEST_PARAMETER_FT_TX C WHERE C.SID = "
//					+ sid
//					+ " \n"
//					+ "AND C.PGM_ID IN (\n"
//					+ "SELECT A.PREVIOUS_PROGRAM_ID FROM PG_PREVIOUS_PROGRAM A WHERE A.PROGRAM_ID IN ( \n"
//					+ "SELECT DISTINCT(B.PGM_ID) FROM TF_TEST_PARAMETER_FT_TX  B WHERE B.SID = "
//					+ sid + ") \n" + "AND A.PROGRAM_MODE = 'PROD' \n" +
//					// "AND A.CATEGORY IN (1,2) \n" + 連Program to be expired
//					// 也要納入檢查
...
//	}
//
//	public static String doublicatePIMS(int sid) {
//		Connection conn = null;
//		StringBuffer s = new StringBuffer();
//		try {
//			conn = DBConnection.getConnection();
//			StringBuffer sql = new StringBuffer();
//			sql.append("SELECT DISTINCT PGM_ID FROM TF_TEST_PARAMETER_FT_TX C WHERE C.SID = "
//					+ sid
//					+ " \n"
//					+ "AND C.PGM_ID IN (\n"
//					+ "SELECT A.PREVIOUS_PROGRAM_ID FROM PG_PREVIOUS_PROGRAM A WHERE A.PROGRAM_ID IN ( \n"
