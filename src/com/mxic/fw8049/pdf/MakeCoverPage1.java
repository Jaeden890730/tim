...
import com.mxic.oiplus.pdf.MakePDFDiff;
import com.mxic.oiplus.pdf.PageNumbersWatermark;
...
public class MakeCoverPage1 {
  ...
  public static boolean makePDFCoverPage(Fw8049MainActionForm fm, String vendor, String status) {
    boolean IdFlag = true;
...
/*所需參數為ProTestRouteBeanAF(ActionForm)與檔案名稱(String)*/
  public static boolean makePDFCoverPage(Connection conn, Fw8049MainActionForm fm, String vendor, String status) {
    boolean IdFlag = true;
    //Connection conn = null;
    ...
    try {
      ...
      //conn = DBConnection.getConnection();
      if (status.equals("R"))
      	fileName = pdfService.getFileName(conn, pro_b, br, version, vendor, true, "");
      else
      	fileName = pdfService.getFileName(conn, pro_b, br, version, vendor, true, "_tx");
      ...
      document.newPage();
      if (vendor == null) {
    	  MakePDFVendorDiff.makePDF(conn, fm, document, status);
    	  document.newPage();
    	  
    	  MakePDFDiff.makePDF(conn, fm.getSid(), fm.getProduct_body(), fm.getBrand(), fm.getVersion(), status, document);
      }
/*
      else {
